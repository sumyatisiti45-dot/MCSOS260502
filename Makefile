...
smoke:
	...
	readelf -h $(BUILD_DIR)/smoke/freestanding.o \
		| tee $(BUILD_DIR)/smoke/readelf-header.txt

	objdump -drwC $(BUILD_DIR)/smoke/freestanding.o \
		| tee $(BUILD_DIR)/smoke/objdump.txt >/dev/null

	file $(BUILD_DIR)/smoke/freestanding.o \
		| tee $(BUILD_DIR)/smoke/file.txt
