...
| R-M0-003 | QEMU tidak tersedia | Medium | High | Pasang `qemu-systemx86`; catat versi | Toolchain engineer | `command -v qemu-system-x86_64` gagal |
...
| R-M0-005 | Compiler menghasilkan target host | Medium | High | Pakai `-- target`; inspeksi `readelf` | Verification engineer | `Machine` bukan x8664 |
